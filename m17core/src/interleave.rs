...
pub fn interleave(payload: &[u8]) -> [u8; 46] {
    let payload_bits = Bits::new(payload);
    let mut new = [0; 46];
    let mut new_bits = BitsMut::new(&mut new);
    for (input, output) in MAPPING.iter().enumerate() {
        new_bits.set_bit(*output, payload_bits.get_bit(input));
    }
    new
}
...
mod tests {
    ...
    #[test]
    fn interleaving_table() {
        let mut mapping = [0; 368];
        for i in 0..368i64 {
            mapping[i as usize] = ((45 * i + 92 * i.pow(2)) % 368) as usize;
        }
        println!("{mapping:?}");
